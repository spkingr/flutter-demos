...
class NinjaComponent extends BodyComponent {
  ...
  void update(double t) {
    this.idle = body.linearVelocity.x.abs() <= double.minPositive;
    this.forward = body.linearVelocity.x > double.minPositive;
  ...
  void _createBody() {
    ...
    activeBodyDef.bullet = false;

    this.body = world.createBody(activeBodyDef)
      ..createFixtureFromFixtureDef(activeFixtureDef);
  }
...
class HandleNinjaDrag extends Drag {
  ...
  void impulse(Offset velocity) {
    Vector2 force = new Vector2(velocity.dx, -velocity.dy)..scale(10.0);//100
